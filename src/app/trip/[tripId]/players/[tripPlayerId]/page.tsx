...
export default async function PlayerProfilePage({
  ...
  params: Promise<{ tripId: string; tripPlayerId: string }>
}) {
  const { tripId, tripPlayerId } = await params
  ...
  // Fetch trip for back-link label
  ...
  // Fetch player info — name, handicap, profile
  ...
  if (!tripPlayer) notFound()

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const playerRaw = Array.isArray(tripPlayer.player) ? tripPlayer.player[0] : tripPlayer.player as any
  const playerId = playerRaw?.id as string | undefined
  const playerName = (playerRaw?.name as string) || 'Unknown'
  const handicapIndex = playerRaw?.handicap_index as number | null
  const profileUserId = playerRaw?.user_id as string | null

  // Resolve display name / avatar / bio from player_profiles
  ...
  // ── All trip_players for this player (used by earnings, map, and friends) ──
  ...
  const { data: roundStats } = await supabase
    ...
    .select(`
      ...
      par_total,
      holes_played,
      course:courses!inner(id, name, par, round_number, round_date, trip_id)
    `)
  ...
  const rounds = (roundStats || [])
    ...
    .filter((r) => r.tripId === tripId && r.gross != null)
    .sort((a, b) => a.roundNumber - b.roundNumber)

  // ── Map Pins (all trips — courses with coordinates) ────────────────────────
  ...
  const mySideMap = new Map((myMatchPlayers || []).map((mp) => [mp.match_id, mp.side]))
  ...
  if (matchIds.length > 0) {
    const { data: matches } = await supabase
      ...
      .select(`
        ...
        status,
        course_id,
        course:courses!inner(name, round_number, round_date, trip_id),
        match_players(
          trip_player_id,
          side,
    ...
    for (const match of matches || []) {
      ...
      if (course?.trip_id !== tripId) continue

      const mySide = mySideMap.get(m.id)
  ...
  if (profileUserId) {
    ...
    if (isOwnProfile && currentUserId) {
      // Fetch all friendships for this user at once
      ...
      // Fetch all relevant profiles in one batch
      ...
      // Suggestions: co-players from shared trips not already in a friendship
      ...
      // Viewing someone else's profile — fetch their accepted friends + viewer's friendship status
      const [{ data: profileFships }, viewerFshipData] = await Promise.all([
        supabase
          .from('friendships')
